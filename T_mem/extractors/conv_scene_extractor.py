"""Conversation scene extractor: boundary detection + scene memory generation."""
...
from T_mem.prompts.scene_prompts import CONV_BOUNDARY_DETECTION_PROMPT
from T_mem.prompts.scene_prompts import (
    SCENE_GENERATION_PROMPT,
    DEFAULT_CUSTOM_INSTRUCTIONS,
)
from T_mem.extractors.scene_extractor import (
    SceneExtractor, RawData, StatusResult, SceneExtractRequest
)
...
@dataclass
class ConvSceneExtractRequest(SceneExtractRequest):
    pass


class ConvSceneExtractor(SceneExtractor):
    ...
    async def _generate_scene_memory(self, scene: Scene, message_dict_list: List[Dict[str, Any]]) -> Scene:
        conversation_text = self._format_conversation_json_text(message_dict_list)

        start_time = self._parse_timestamp(scene.timestamp)
        ...
        if "title" not in data:
            data["title"] = "Conversation Scene"
        if "content" not in data:
            data["content"] = conversation_text
        if "summary" not in data:
            data["summary"] = data["content"][:200] + "..."

        scene.subject = data["title"]
        scene.summary = data["summary"]
        scene.scene_description = data["content"]

        return scene


    async def extract_scene(self, request: ConvSceneExtractRequest) -> tuple[Optional[Scene], Optional[StatusResult]]:
        ...
        if should_end:
            ...
            for attempt in range(max_retries):
                try:
                    logger.debug(f"Triggering scene memory generation (attempt {attempt + 1}/{max_retries})")
                    now = time.time()
                    scene = await self._generate_scene_memory(scene, history_message_dict_list)
                    logger.debug(f"Scene memory generation completed in {time.time() - now:.2f}s")
                    logger.info("Scene memory generated successfully")
                    return (scene, status_control_result)
                except Exception as e:
                    logger.error(f"Scene memory generation failed: {e} (attempt {attempt + 1}/{max_retries})")

                if attempt < max_retries - 1:
                    await asyncio.sleep(0.5)
                else:
                    logger.error("All retries failed for scene memory generation")
